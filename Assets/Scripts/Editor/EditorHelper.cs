...
public static class EditorHelper {
    ...
    //private static Dictionary<TrophyID, GMTrophy> _trophies;

    //public static TrophyID TrophyEnumPopup(string label, TrophyID value, bool drawStatusIcon = true, params GUILayoutOption[] options) {
    //    GMTrophy trophy = null;
    //    return TrophyEnumPopup(label, value, ref trophy, drawStatusIcon, options);
    //}

    //public static TrophyID TrophyEnumPopup(string label, TrophyID value, ref GMTrophy usedTrophy, bool drawStatusIcon = true, params GUILayoutOption[] options) {
    //    if (drawStatusIcon && _trophies == null) {
    //        EditorUtility.DisplayProgressBar("Loading trophies for enum popup...", "...", 0f);

    //        GMTrophy[] trophies = Resources.LoadAll<GMTrophy>("Trophies");
    //        _trophies = new Dictionary<TrophyID, GMTrophy>(trophies.Length);
    //        foreach (GMTrophy trophy in trophies)
    //            if (trophy.trophyID != TrophyID.None && trophy.trophyID != TrophyID.All && !_trophies.ContainsKey(trophy.trophyID))
    //                _trophies.Add(trophy.trophyID, trophy);
    ...
    //    TrophyID result = EnumPopup(label, value, true, null, options);

    //    if (drawStatusIcon) {
    //        if (result == TrophyID.None || result == TrophyID.All) {
    //            IconDisabled("No Trophy");
    //        }
    //        else if (_trophies.ContainsKey(result)) {
    //            usedTrophy = _trophies[result];

    //            if (usedTrophy == null) // trophy got deleted
    //            {
    //                _trophies = null;
    //                ButtonError("Trophy Expired!");
    //            }
    //            else {
    //                if (usedTrophy.gameObject.CompareTag("EditorOnly")) {
    //                    if (ButtonWarning("Editor-Only Trophy Found (click to select)"))
    //                        Selection.activeObject = _trophies[result];
    //                }
    //                else if (ButtonOkay("Trophy Found (click to select)"))
    //                    Selection.activeObject = _trophies[result];
    //            }
    //        }
    //        else {
    //            if (ButtonError("Trophy Not Found (click to refresh)"))
    //                _trophies = null;
    //        }
    //    }
    ...
    //private static Dictionary<CraftingComponentID, CraftingComponent> _components;

    //private static List<CraftingComponentID> _badComponentEntries = new List<CraftingComponentID>()
    //{
    //    CraftingComponentID.None,
    //    CraftingComponentID.All
    //};

    //public static CraftingComponentID CraftingComponentEnumPopup(string label, CraftingComponentID value, bool drawStatusIcon = true, bool reddenBadValues = true, params GUILayoutOption[] options) {
    //    CraftingComponent component = null;
    //    return CraftingComponentEnumPopup(label, value, ref component, drawStatusIcon, reddenBadValues, options);
    //}

    //public static CraftingComponentID CraftingComponentEnumPopup(string label, CraftingComponentID value, ref CraftingComponent relevantComponent, bool drawStatusIcon = true, bool reddenBadValues = true, params GUILayoutOption[] options) {
    //    if (drawStatusIcon && _trophies == null) {
    //        CraftingComponent[] components = Resources.LoadAll<CraftingComponent>("CraftingComponents");
    //        _components = new Dictionary<CraftingComponentID, CraftingComponent>(components.Length);
    //        foreach (CraftingComponent component in components)
    //            if (component.componentID != CraftingComponentID.None && component.componentID != CraftingComponentID.All && !_components.ContainsKey(component.componentID))
    //                _components.Add(component.componentID, component);
    //    }

    //    EditorGUILayout.BeginHorizontal();

    //    CraftingComponentID result = EnumPopup(label, value, reddenBadValues ? _badComponentEntries : null, options);

    //    if (drawStatusIcon) {
    //        if (result == CraftingComponentID.None || result == CraftingComponentID.All) {
    //            IconDisabled("No Component");
    //        }
    //        else if (_components == null) {
    //            //TODO: Something better than just ignoring it...
    //        }
    //        else if (_components.ContainsKey(result)) {
    //            relevantComponent = _components[result];

    //            if (ButtonOkay("Component Found (click to select)"))
    //                Selection.activeObject = _components[result];
    //        }
    //        else {
    //            if (ButtonError("Component Not Found (click to refresh)"))
    //                _trophies = null;
    //        }
    //    }
